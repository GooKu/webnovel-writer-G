...
初始化時應同時產出 `novel.config.json`（複製自 [novel-config/novel.config.sample.json](../novel-config/novel.config.sample.json)，依實際目錄結構填寫）。後續所有 skill 將透過 `scripts/config_resolver.py` 讀此 config 解析路徑；未生成 config 時回退到 webnovel-writer-G 預設路徑。完整對照見 [novel-config/PATH_MAPPING.md](../novel-config/PATH_MAPPING.md)。
